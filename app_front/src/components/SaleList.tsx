...
const SaleList: React.FC = () => {
    ...
    return (
        <div>
                    ...
                    {sales.map((sale: any) => (
                        <tr key={sale.id}>
                            ...
                            <td className="text-center py-2">
                                <Link to={`/sales/${sale.id}`} className="text-blue-500 hover:text-blue-700">
                                    <FontAwesomeIcon icon={faEye} />
                                </Link>
                                <Link to={`/sales/${sale.id}/edit`} className="text-yellow-500 hover:text-yellow-700 ml-2">
            ...
            <Link to="/sales/new" className="bg-gray-500 text-white px-4 py-2 rounded hover:bg-gray-700 mt-4 inline-block">Añadir Venta</Link>
        </div>
    );
};
